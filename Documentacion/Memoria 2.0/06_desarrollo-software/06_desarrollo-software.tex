...
\begin{figure}[H]
  \begin{center}
    \includegraphics[scale=0.04]{diagramas/subsistemas.png}
  \end{center}
...
Diagrama de bloques \ref{figura:diagrama-comunicacion-robot} representativo de los diferentes canales de comunicaciones abiertos por SensorRS que irán describíendose en los sucesivos puntos.
...
Si, en cambio, para el vehículo que deseemos programar resultan necesarios más pines para la utilización de servos, sensores o cualquier otro elemento utilizando los pines GPIO de
la placa Raspberry Pi, tan solo debemos inicializarlos e indicar si van a ser pines de entrada o de salida. Si existen dudas al respecto se puede acceder a la documentación de
...
\begin{lstlisting}[language=JavaScript]
  ...
  gpio4.on('alert', (level, tick) => {
    console.log('Activo!');
  });  //Activacion del evento en pin 4
\end{lstlisting}
...
Para la utilización del módulo \emph{SerialPort} debemos conocer y especificar el puerto serie asignado a nuestra placa Arduino. Para ello abrimos una terminal en nuestra Raspberry Pi
 con la placa arduino conectada e introducimos el siguiente comando:\\

\begin{lstlisting}[language=bash]
ls /dev | grep ttyACM
\end{lstlisting}

Una vez conocido el puerto, lo emplearemos para la inicialización de la conexión con el puerto serie.\\
...
El envío de cada caracter string irá acompañado de un retraso, \emph{delay}, el cual es introducido por parámetro.\\
...
A continuación se muestra el código para la recepción del mensaje desde la placa Arduino, mostrándose como ejemplo el código referente al control de motores:\\
...
#define E2 13  // Enable Pin para motor 2
#define D1 12  // Control pin 1 para motor 2
#define D2 11 // Control pin 2 para motor 2
...
void setup() {
  ...
  // Establece direccion de rotacion inicial
  digitalWrite(I1, LOW);
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (received == '\n') {  //Caracter final recibido, mensaje capturado
      inData.trim();

      //Direccion
      if (inData.startsWith("SRV")){ //Comienzo del string identifica el tipo de comando. Accionamiento de la direccion
      ...
      //MOTOR
      if (inData.startsWith("MOTOR")){ //Comando accionador del motor
      ...
      inData = ""; //Fin de procesamiento, borrado del mensaje.
    }
  }
...
A continuación se muestra un ejemplo de envío de la lectura de los calores de temperatura y humedad del sensor Dth11 conectado al Arduino para su transferencia a la Placa Raspberry Pi:\\
